...
class TestConversationTree:
    def test_conversation_links(self):
        ...
        def verify_node(node):
            for link in node.link:
                assert len(link.name) == 0 or link.name in valid_ids, f"node['{node.name}'].link['{link.name}'] links to node that does not exist."
        for node in conversation.node:
    ...
    def test_answers(self):
        ...
        def do_answer_validation(node):
            ...
            assert len(node.answer) > 0, f"node['{node.name}'] must have at least one answer."
            for answer in node.answer:
                if answer.WhichOneof("answer") == "links":
                    assert len(answer.links.text) > 0, f"node['{node.name}'].answer.links must have text set."
                    index = 0
                    for url in answer.links.url:
                        assert len(url.label) > 0 and len(url.url) > 0, f"node['{node.name}'].answer.links[{index}].url must have both label and url set."
                    ...
                    assert os.path.exists("photo/%s" % answer.photo), f"node['{node.name}'].answer.photo['{answer.photo}'] photo does not exist."
