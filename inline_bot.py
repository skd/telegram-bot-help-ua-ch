...
NO_SUCH_NODE = "I did not understand."
...
def start_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            CHOOSING: [CallbackQueryHandler(choice)],
        },
        fallbacks=[
    ...
    for name in node_by_name:
        if len(node_by_name[name].link) > 0:
            ...
            for link in node_by_name[name].link:
                if len(link.name) > 0:
                    options.append([InlineKeyboardButton(link.name, callback_data=link.name)])
            ...
            if name != START_NODE:
                options.append([InlineKeyboardButton(START_OVER, callback_data=START_OVER)])
            keyboard_by_name[name] = InlineKeyboardMarkup(options)
